...
# simple gam
require(mgcv)
baseModel<- gam(ducks ~ Depth + as.factor(phase), family=poisson, data=data)

# gam with a smooth on depth
baseModel<- gam(ducks ~ s(Depth, k=4) + as.factor(phase), family=poisson, data=data)
...
for(i in 1:nsim){
  ...
  dataObj<-wrapperfunc(baseModel, grid2, data, seed=i)
